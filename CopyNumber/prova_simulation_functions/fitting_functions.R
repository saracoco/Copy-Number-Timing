fit_model_selection_best_K <- function(all_sim, karyo, purity = 0.95, max_attempts = 10, INIT = TRUE, simulation_params) {
  if (INIT == TRUE) {
  ...
  for (K in 1:length(karyo)) {
    ...
    # Funzione per calcolare dinamicamente nrow e ncol
    calculate_nrow_ncol <- function(num_facets) {
      nrow <- ceiling(sqrt(num_facets))
      ncol <- ceiling(num_facets / nrow)
      return(list(nrow = nrow, ncol = ncol))
    }
    
    # Calcolare nrow e ncol dinamicamente
    num_facets <- length(unique(simulation_params$Karyo))
    dims <- calculate_nrow_ncol(num_facets)
    
    # Creare il grafico
    # param_plot <- ggplot(simulation_params, aes(x = factor(Parameter), y = Value)) +
    #   geom_bar(stat = "identity", fill = "blue") +
    #   labs(title = "Simulation Parameters", x = "Parameter", y = "Value") +
    #   facet_wrap(~ vector_karyo[[1]], scales = "free_y", nrow = dims$nrow, ncol = dims$ncol)
    # 
    
    
    p <- plotting(res, input_data, K, simulation_params=simulation_params) + plot_data + param_plot
    ggsave(paste0("./plots/plot_inference_", all_sim$j[1], "_", K, ".png"), width = 12, height = 24, device = "png", plot = p)
  }
  ...
  p_best_K <- plotting(res, input_data, best_K, simulation_params = simulation_params) + plot_data + param_plot
  ggsave(paste0("./plots/plot_inference_", all_sim$j[1], "_", best_K, ".png"), width = 12, height = 24, device = "png", plot = p_best_K)
