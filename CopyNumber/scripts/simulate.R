...
for(i in 1:n_simulations){
  ...
  for (j in 1:K){
    ...
    if (j != 1){
        vector_tau[j] = runif(1, 0)
      while (abs(vector_tau[j-1] - vector_tau[j]) < 0.20){
        vector_tau[j] = runif(1, 0)
      }
    }
  }
  ...
  log_lik_plot <- ggplot(data = model_selection, aes(x = K, y = Log_lik)) + 
    ...
    geom_point(aes(x = K[Log_lik == min(Log_lik)], y = min(Log_lik), colour = "Minimum Point")) +
    scale_colour_manual(name = "Legend",
  ...
  saveRDS(model_selection_plot, "model_selection.rds")
